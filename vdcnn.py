...
def convolutional_block(inputs, shortcut, num_filters, name, is_training, keep_prob):
    # print("-" * 20)
    ...
    with tf.variable_scope("conv_block_" + str(num_filters) + "_" + name):
        for i in range(2):
            with tf.variable_scope("conv1d_%s" % str(i)):
                ...
                if i == 1:
                    tf.nn.dropout(inputs, keep_prob=keep_prob)
...
class VDCNN():
    def __init__(self, num_classes, sequence_max_length=1024, num_quantized_chars=69, embedding_size=16,
                 depth=9, downsampling_type='maxpool', use_he_uniform=True, optional_shortcut=False, keep_prob=0.6,
        ...
        for i in range(num_layers[0]):
            ...
            conv_block = convolutional_block(inputs=self.layers[-1], shortcut=shortcut, num_filters=64,
                                             is_training=self.is_training, name=str(i + 1), keep_prob=keep_prob)
        ...
        # Get softmax values for each class
        with tf.name_scope("softmax_output"):
            self.softmax_output = tf.nn.softmax(logits=self.fc3)

        # Calculate Mean cross-entropy loss
        with tf.name_scope("loss"):
            self.predictions = tf.argmax(self.fc3, 1, name="predictions")
            losses = tf.nn.weighted_cross_entropy_with_logits(logits=self.fc3, targets=self.input_y, pos_weight=classes_weights)

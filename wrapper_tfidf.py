...
import pandas as pd
import os
from ngrams_tfidf import train_tfidf_ngrams, predict_label_tfidf_ngrams


def wrapper(ratio, path, balanced):
    ...
    while runMore:
        model, vectorizer = train_tfidf_ngrams(path)
        predict_label_tfidf_ngrams(path, model, vectorizer)
    ...
    for i in range(0, len(datasets)):
        for size_folder in size_folders:
            for j in range(len(ratio_folders)):
                wrapper(ratios[j], 'slp_ngrams_tfidf/' + datasets[i] + size_folder + ratio_folders[j], balanced)
